...
public class Original {
    ...
    public int tallestBillboard(int[] rods) {
        ...
        final Map<Integer, Integer>[] dp = new HashMap[n + 1];
        for (int i = 0; i <= n; i++)
            dp[i] = new HashMap<>();

        return recursiveTravers(rods, dp, 0, 0);
    }

    private int recursiveTravers(int[] rods, Map<Integer, Integer>[] dp, int index, int height) {
        ...
        int v1 = recursiveTravers(rods, dp, index + 1, height);
        int v2 = rods[index] + recursiveTravers(rods, dp, index + 1, height - rods[index]);
        int v3 = recursiveTravers(rods, dp, index + 1, height + rods[index]);
