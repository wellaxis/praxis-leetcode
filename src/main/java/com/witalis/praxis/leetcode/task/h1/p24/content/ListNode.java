package com.witalis.praxis.leetcode.task.h1.p24.content;
...
public class ListNode {
    ...
    public static ListNode initList(int number) {
        ...
        ListNode top = new ListNode(random.nextInt(0, number));
        ListNode node = top;
        while (size != 0) {
            node.next = new ListNode(random.nextInt(0, number));
            node = node.next;
            size--;
        }
        return top;
