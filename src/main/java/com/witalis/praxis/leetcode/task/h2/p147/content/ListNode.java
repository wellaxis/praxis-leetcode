package com.witalis.praxis.leetcode.task.h2.p147.content;
...
public class ListNode implements Cloneable {
    ...
    public static ListNode initList(int length, int value) {
        ...
        var size = random.nextInt(0, length);

        ListNode top = new ListNode(random.nextInt(-value, value + 1));
        ListNode node = top;
        while (size != 0) {
            node.next = new ListNode(random.nextInt(-value, value + 1));
            node = node.next;
            size--;
        }
        return top;
