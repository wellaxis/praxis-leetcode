package com.witalis.praxis.leetcode.task.h4.p303.content;

import lombok.extern.slf4j.Slf4j;
import java.util.AbstractMap;
import java.util.ArrayList;
import java.util.List;

/**
 * Desc: Numeric operations handler
 * User: Wellaxis
 * Date: 4/15/2022
 */
@Slf4j
public final class NumericHandler {

    private NumericHandler() {
        super();
    }

    public static List<Integer> process(
        List<AbstractMap.SimpleEntry<String, int[]>> operations,
        NumericArray numericArray
    ) {
        List<Integer> results = new ArrayList<>();

        NumericArray array = null;

        for (AbstractMap.SimpleEntry<String, int[]> operation: operations) {
            var name = operation.getKey();
            var value = operation.getValue();
            switch (name) {
                case "NumArray" -> {
                    assert array == null;
                    array = numericArray;
                    results.add(null);
                }
                case "sumRange" -> {
                    assert array != null;
                    int sum = array.sumRange(value[0], value[1]);
                    results.add(sum);
                }
                default -> throw new UnsupportedOperationException(
                    "Unrecognisable operation has been detected: " + operation
                );
            }
        }

        return results;
    }
}
