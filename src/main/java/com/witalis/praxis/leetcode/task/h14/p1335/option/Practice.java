...
public class Practice {
    ...
    public Practice(int[] jobDifficulty, int days) {
    ...
    public int minDifficulty(int[] jobDifficulty, int d) {
        ...
        this.dp = new int[PERIOD + 2][LEN + 2];
        for (int i = 0; i <= d; i++)
            Arrays.fill(dp[i], -1);

        int minDifficulty = dfs(jobDifficulty, d, 0);

        return minDifficulty == Integer.MAX_VALUE ? -1 : minDifficulty;
    }

    private int dfs(int[] jobDifficulty, int days, int index) {
        ...
        for (int i = index; i < jobDifficulty.length; i++) {
            ...
            int rest = dfs(jobDifficulty, days - 1, i + 1);
