...
public class Original {
    ...
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        for (int course = 0; course < numCourses; course++) {
            if (!processed[course]) {
                if (!recursiveRequisites(requisites, courses, course, new HashSet<>(), processed)) return new int[0];
            }
        }
    ...
    private boolean recursiveRequisites(
        ...
        for (int next : requisites.get(course)) {
            if (!processed[next]) {
                if (!recursiveRequisites(requisites, courses, next, chain, processed)) return false;
            }
        }
        chain.remove(course);
