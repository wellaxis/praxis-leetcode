...
public class Original {
    ...
    public List<List<Integer>> combinationSum(int[] candidates, int target) {
        ...
        reverseCombination(target, options, combinations, new Stack<>(), 0);
        return new ArrayList<>(combinations);
    }

    private void reverseCombination(int target, List<Integer> options, Set<List<Integer>> combinations, Stack<Integer> combination, int j) {
        if (target < 0) {
            return;
        } else if (target == 0) {
            ...
            for (int i = j; i < options.size(); i++) {
                ...
                reverseCombination(target, options, combinations, combination, i);
