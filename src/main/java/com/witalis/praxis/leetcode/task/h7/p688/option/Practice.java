...
public class Practice {
    ...
    public Practice(int size, int moves, int row, int column) {
        this.size = size;
        this.moves = moves;
        this.row = row;
        this.column = column;
    ...
    public double knightProbability(int n, int k, int row, int column) {
        ...
        for (int r = 0; r < n; r++)
            for (int c = 0; c < n; c++)
                probability += recursiveProbability(n, k, row, column, r, c);

        return probability;
    }

    private double recursiveProbability(int n, int k, int row, int column, int r, int c) {
        if (k == 0) return (r == row && c == column) ? 1 : 0;
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (dr >= 0 && dr < n && dc >= 0 && dc < n) {
                dp[k][r][c] += recursiveProbability(n, k - 1, row, column, dr, dc) / 8D;
            }
        }
