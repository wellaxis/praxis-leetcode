...
public class Practice {
    ...
    public Practice(int size, int edge, int cost) {
        this.size = size;
        this.edge = edge;
        this.cost = cost;
    }
    ...
    public int numOfArrays(int n, int m, int k) {
        ...
        return solver(n, m, k, 0);
    }

    int solver(int n, int m, int k, int i) {
        if (n <= 0) {
            if (k == 0) return 1;
            return 0;
        }
        ...
        for (int j = 1; j <= m; j++) {
            if (j > i) {
                if (k >= 1) {
                    ans = (ans + solver(n - 1, m, k - 1, j)) % MOD;
                }
            } else {
                ans = (ans + solver(n - 1, m, k, i)) % MOD;
            }
        }
        dp[i][n][k] = ans % MOD;
